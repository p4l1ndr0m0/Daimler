...
class avisosViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, HomeAvisosProtocal {
   
    
    var feedItems: NSArray = NSArray()
    var selectedLocation : LocationAvisos = LocationAvisos()
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        // Retrieve cell
        let cellIdentifier: String = "avisosCell"
        let myCell: UITableViewCell = tableView.dequeueReusableCellWithIdentifier(cellIdentifier)!
        // Get the location to be shown
        let item: LocationAvisos = feedItems[indexPath.row] as! LocationAvisos
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        selectedLocation = feedItems[indexPath.row] as! LocationAvisos
        // Manually call segue to detail view controller
        self.performSegueWithIdentifier("segueavisos", sender: self)
    }
    
    override func prepareForSegue(segue: UIStoryboardSegue?, sender: AnyObject?) {
        if (segue!.identifier == "segueavisos"){
            let detailVC  = segue!.destinationViewController as! avisoViewController
